...
1. 每天北京时间 09:30，Codex 启动一个全新的独立任务会话，完整读取双频道模板和 `docs/daily-runbook.md`，以长时目标执行搜索与权威来源回退，不复用上一次运行的聊天上下文。
2. 检索过程增量写入当天 `run-state.json` 与 `research-ledger.jsonl`；10:50 后只进行发布前校验和必要修正。两个频道独立校验，一个频道失败不会阻塞另一个频道。
...
6. 11:10 以另一个全新独立任务会话运行健康检查，读取当天结构化账本并只续做缺失栏目、失败频道或失败部署；再次失败则保留上一期并通知人工处理。

每次发布还必须通过日期一致性和来源完整性门禁：归档日期、正文日期、期号、文件名和 11:00 发布时间必须完全对应；游戏资讯只能来自日报当天或前一天，新一期配图必须使用以当天日期开头的独立文件名；Minecraft 整合包保留真实发布日期，但必须提供当天或前一天的中国社区热度证据；Steam 优惠新期次固定为 6—24 项。两个频道都会与最近 7 期比对标题和原文链接，超过栏目允许的重复上限即拒绝发布。11:10 健康检查与 GitHub Pages 部署复用同一套门禁。
