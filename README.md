...
1. 每天北京时间 08:30、09:30、10:30，Codex 分三次启动全新的独立制作任务，完整读取双频道模板和 `docs/daily-runbook.md`，按当天结构化状态接力，不复用上一次运行的聊天上下文。
...
每次发布还必须通过日期一致性、来源完整性和就绪证明门禁：归档日期、正文日期、期号、文件名和 11:00 发布时间必须完全对应；候选JSON、渲染HTML、渲染PNG与公开PNG的哈希必须匹配，PNG宽度必须为3840px；游戏资讯只能来自日报当天或前一天，新一期配图必须使用以当天日期开头的独立文件名；Minecraft 整合包保留真实发布日期，但必须提供当天或前一天的中国社区热度证据；Steam 优惠新期次固定为 6—24 项。两个频道都会与最近 7 期比对标题和原文链接，超过栏目允许的重复上限即拒绝发布。11:01/11:31 健康检查与 GitHub Pages 部署复用同一套门禁。
