...
1. 每天北京时间 05:00—10:00 每小时整点运行游戏频道制作任务，05:30—10:30 每小时半点运行民生频道制作任务。两个专职 Codex 任务错峰接力，分别读取本频道模板和 `docs/daily-runbook.md`，按当天结构化状态续跑，不复用上一次运行的聊天上下文。
2. 每轮先取得自动过期运行租约，防止并发修改；检索过程增量写入当天 `run-state.json` 与 `research-ledger.jsonl`，10:50 后只进行发布前校验和必要修正。两个频道独立校验，一个频道失败不会阻塞另一个频道。
...
6. 11:01与11:31以另一个全新独立任务运行发布健康检查和精确补跑：第一次在11:00门禁开放后立即正常发布，第二次只重试明确失败项；11:31后仍失败才保留上一期并通知人工。

每次发布还必须通过日期一致性、来源完整性和就绪证明门禁：归档日期、正文日期、期号、文件名和 11:00 发布时间必须完全对应；候选JSON、渲染HTML、渲染PNG与公开PNG的哈希必须匹配，PNG宽度必须为3840px；游戏资讯只能来自日报当天或前一天，新一期配图必须使用以当天日期开头的独立文件名；Minecraft 整合包保留真实发布日期，但必须提供当天或前一天的中国社区热度证据；Steam 网页收录当天检索并核验出的全部合格热门优惠，不设24项上限，静态PNG只展示排序前6项。两个频道都会与最近 7 期比对标题和原文链接；持续有效的 Steam 优惠允许跨日出现，但必须由当天完整检索账本重新核验。11:01/11:31 健康检查与 GitHub Pages 部署复用同一套门禁。

统一来源名称、别名和栏目必查集合位于 `config/daily-sources.json`。运行与补跑命令、审计快照规则及健康检查决策顺序见 `docs/daily-runbook.md`。
