...
6. 11:10与11:40以另一个全新独立任务运行发布健康检查和精确补跑：第一次负责正常发布，第二次只重试明确失败项；11:40后仍失败才保留上一期并通知人工。

每次发布还必须通过日期一致性、来源完整性和就绪证明门禁：归档日期、正文日期、期号、文件名和 11:00 发布时间必须完全对应；候选JSON、渲染HTML、渲染PNG与公开PNG的哈希必须匹配，PNG宽度必须为3840px；游戏资讯只能来自日报当天或前一天，新一期配图必须使用以当天日期开头的独立文件名；Minecraft 整合包保留真实发布日期，但必须提供当天或前一天的中国社区热度证据；Steam 优惠新期次固定为 6—24 项。两个频道都会与最近 7 期比对标题和原文链接，超过栏目允许的重复上限即拒绝发布。11:10/11:40 健康检查与 GitHub Pages 部署复用同一套门禁。
