...
- 禁止 OpenAI Platform API、`OPENAI_API_KEY`、`api.openai.com` 和任何付费模型 API。每日研究使用当前 Codex 任务提供的网页检索。用户于2026-09-27明确授权：仅对 Steam 官方页面，网页工具不可用时可使用项目只读浏览器备用采集；保留原始响应、真实时间、哈希与08:00冻结门禁，其他来源不因此改用本地抓取。
